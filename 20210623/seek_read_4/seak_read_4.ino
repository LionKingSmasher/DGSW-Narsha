...
char buf_full_or_not(){
  if(gcode_buf_head > gcode_buf_tail){
    if(gcode_buf_head - gcode_buf_tail > 1) return NOT_FULL;
    else return FULL;
  }
  else if(gcode_buf_head < gcode_buf_tail) {
    int tmp_buf_len = gcode_buf_head - gcode_buf_tail + GCODE_BUF_LEN;
    if(tmp_buf_len > 1) return NOT_FULL;
    return FULL;
  }
  else if(gcode_buf_head == gcode_buf_tail)
    return NOT_EMPTY;
...
void loop() {
  if (myFile) {
    if(myFile.available()){
      gcode_buf_full = buf_full_or_not();
      if(gcode_buf_full == 0){
        while(myFile.available()){
          ...
          if(tmp_read == '\n'){
            ...
            if(gcode_buf_head == gcode_buf_tail){
              ...
              gcode_buf_head--;
              if(gcode_buf_head) 
                gcode_buf_head = GCODE_BUF_LEN - 1;
            }
            delay(1000);
          }
        }
